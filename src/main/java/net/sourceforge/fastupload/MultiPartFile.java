...
public abstract class MultiPartFile {
	...
	public MultiPartFile(String name) throws UnsupportedEncodingException {
		this.name = new String(name.getBytes(), charset);
	}

	public MultiPartFile(String name, String charset) throws UnsupportedEncodingException {
		this.name = new String(name.getBytes(), charset);
	...
	/**
	 * open a {@link InputStream} object for the {@link MultiPartFile} object if
	...
	/**
	 * get all bytes of content in a boundary contains in the {@link MultiPartFile} object
	 * 
	 * @return
	 */
	...
	/**
	 * convert bytes of buffer with default charset specified, if
	 * current boundary header does not contain file information
	 * 
	 * @param charset
	 * @return
